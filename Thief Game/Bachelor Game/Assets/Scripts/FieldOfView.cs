            ...
            foreach (RaycastHit2D hit in raycastHit2D) {
                if(renderUpdater){
                    ...
                    if(hit.collider.CompareTag("Obstacle")||hit.collider.CompareTag("Hidden")){
                        if (hit.transform.gameObject.GetComponent<SpriteRenderer>().sharedMaterial.shader.name != "Sprites/Default")
                        {
                            hit.transform.gameObject.GetComponent<SpriteRenderer>().material.shader = Shader.Find("Sprites/Default");
                        }
                    }
                }
